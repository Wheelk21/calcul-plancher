...
function creerCheckboxes() {
  const container = document.getElementById("planches");
  const actives = chargerPlanchesActives();

  container.innerHTML = CONFIG.planches.map(largeur => `
    <label class="check">
      <input type="checkbox" class="plank-check" value="${largeur}" ${actives.includes(largeur) ? "checked" : ""}>
...
function ordrePose(solution, actives) {
  ...
  for (const largeur of actives) {
    for (let i = 0; i < (solution.counts[largeur] || 0); i++) {
      ordre.push(largeur);
    }
  }
  return ordre;
...
function renderRows(solution) {
  return CONFIG.planches.map(largeur => `
    <div class="row">
      <span>${largeur} mm</span>
      <span class="value">${textePlanche(solution.counts[largeur] || 0)}</span>
    </div>
  `).join("");
}
